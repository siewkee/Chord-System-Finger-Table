...
The Chord system is a structured peer-to-peer network architecture realizing a scalable distributed hash table. In Chord, the
peers are organized as a ring. A position within Chord which is held by a computing node is called an index node or simply a
peer. Each index node has a finger table and, in addition, holds data items associated with its key value (its ID within the
ring) and any other data items associated with keys larger than the previous index node in the Chord ring.
